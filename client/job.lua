...
local function SetWeaponSeries1()
    for k, _ in pairs(Config.SwatArmory.items) do
        if k < 6 then
            Config.SwatArmory.items[k].info.serie = tostring(QBCore.Shared.RandomInt(2) .. QBCore.Shared.RandomStr(3) .. QBCore.Shared.RandomInt(1) .. QBCore.Shared.RandomStr(2) .. QBCore.Shared.RandomInt(3) .. QBCore.Shared.RandomStr(4))
        end
    end
end

local function SetWeaponSeries2()
    for k, _ in pairs(Config.WeaponArmory.items) do
        if k < 6 then
            Config.WeaponArmory.items[k].info.serie = tostring(QBCore.Shared.RandomInt(2) .. QBCore.Shared.RandomStr(3) .. QBCore.Shared.RandomInt(1) .. QBCore.Shared.RandomStr(2) .. QBCore.Shared.RandomInt(3) .. QBCore.Shared.RandomStr(4))
        end
    end
end
    ...
    QBCore.Functions.TriggerCallback("police:GetImpoundedVehicles", function(result)
        ...
        if result == nil then
            ...
            for _ , v in pairs(result) do
                ...
                local vname = icles[v.vehicle].name
...
RegisterNetEvent('qb-police:client:SWATArmoury', function()
    local authorizedItems = {
        label = Lang:t('menu.pol_SWATarmory'),
        slots = 30,
        items = {}
    }
    local index = 1
    for _, armoryItem in pairs(Config.SwatArmory.items) do
        for i=1, #armoryItem.authorizedJobGrades do
            if armoryItem.authorizedJobGrades[i] == PlayerJob.grade.level then
                authorizedItems.items[index] = armoryItem
                authorizedItems.items[index].slot = index
...
RegisterNetEvent('qb-police:client:WeaponArmoury', function()
    local authorizedItems = {
        label = Lang:t('menu.pol_Weaponarmory'),
        slots = 60,
        items = {}
    }
    local index = 1
    for _, armoryItem in pairs(Config.WeaponArmory.items) do
        for i=1, #armoryItem.authorizedJobGrades do
            if armoryItem.authorizedJobGrades[i] == PlayerJob.grade.level then
                authorizedItems.items[index] = armoryItem
                authorizedItems.items[index].slot = index
...
if Config.UseTarget then
    CreateThread(function()
        ...
        for k, v in pairs(Config.Locations["fingerprint"]) do
         ...
         -- Armoury
         for k, v in pairs(Config.Locations["Patrol Armory"]) do
        ...
        --SWAT Armory
        for k, v in pairs(Config.Locations["SWAT Armory"]) do
            exports['qb-target']:AddBoxZone("SWATArmory_"..k, vector3(v.x, v.y, v.z), 5, 1, {
                name = "SWATArmory_"..k,
                heading = 11,
                debugPoly = false,
                minZ = v.z - 1,
                ...
                options = {
                    {
                        type = "client",
                        event = "qb-police:client:SWATArmoury",
                        icon = "fas fa-swords",
                        label = "SWAT Armory",
                        job = "police",
                    },
        ...
        --Weapon Armory
        for k, v in pairs(Config.Locations["Weapon Armory"]) do
            exports['qb-target']:AddBoxZone("WeaponArmory_"..k, vector3(v.x, v.y, v.z), 5, 1, {
                name = "WeaponArmory_"..k,
                heading = 11,
                debugPoly = false,
                minZ = v.z - 1,
                maxZ = v.z + 1,
                ...
                options = {
                    {
                        type = "client",
                        event = "qb-police:client:WeaponArmoury",
                        icon = "fas fa-swords",
